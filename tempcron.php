...
//if(count($diff)>0) {
	...
	for($i=0;$i<count($xmlList);$i++) {
		...
		echo "searching for: ".$currentXML->destFileName."<br/>";

		
		if(file_exists("./photo/".$currentXML->destFileName)) {
			$storelog=true;
			echo "file found";
...
//$db->CloseConnection();
	...
	function processFile($xml) {
		...
		$_SERVER['DOCUMENT_ROOT'] = dirname(__FILE__);
		echo("now processing".$xml->destFileName);
		...
		$result=$db->Insert(array("mediatype"=>$xml->mediaType,"urn"=>$xml->urn,"post_type"=>$xml->usertype,"filename"=>$xml->destFileName,"publish_result"=>"SUCCESS"),"userphoto",true);
...
function tweetVideoUser($xml) {
	...
	$rewritetwit="https://skytoystory.wassermanexperience.com/twittervideo/";
...
function emailVideoUser($xml) {
	...
	if($xml->mediaType=="jpgfile") {
		$label="photo";
		$link="https://skytoystory.wassermanexperience.com/showphoto.php?urn=";
		$rewriteurl="https://skytoystory.wassermanexperience.com/showphoto/";
		$rewritefb="https://skytoystory.wassermanexperience.com/fbphoto/";
		$rewritetwit="https://skytoystory.wassermanexperience.com/twitterphoto/";
	} else {
		$label="video";
		$link="https://skytoystory.wassermanexperience.com/showvideo.php?urn=";
		$rewriteurl="https://skytoystory.wassermanexperience.com/showvideo/";
		$rewritefb="https://skytoystory.wassermanexperience.com/fbvideo/";
		$rewritetwit="https://skytoystory.wassermanexperience.com/twittervideo/";
	}
...
function emailUser($xml) {
	...
	if($db->records==0) {
		echo "insert new record<br/>";
		$result=$db->Insert(array("usertype"=>$xml->usertype,"personalnote"=>$xml->personalNote,"urn"=>$xml->urn,"current_location"=>$xml->current_location,"firstname"=>$xml->firstname,"lastname"=>$xml->lastname,"email"=>$xml->email,"mobile"=>$xml->mobile,"added"=>$xml->added),"users",true);
	} else {
		echo "update record<br/>";
		$result=$db->Update("users",array("personalnote"=>$xml->personalNote),array("urn"=>$xml->urn));
	}
...
/* Some example calls */
//$connection->get('users/show', array('screen_name' => 'abraham'));
$resulto=$connection->post('statuses/update_with_media', array('status' => 'We love cheering from the stands, come rain or shine! #YouAreFootball','media[]'  => file_get_contents($_SERVER['DOCUMENT_ROOT']."/".$label."/".$xml->destFileName),
"lat"=>$xml->lat,"long"=>$xml->long,"display_coordinates"=>"true"));
